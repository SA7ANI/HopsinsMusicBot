...
from Hopsins.helpers.decorators import errors
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                if message.chat.title.startswith("Channel Music: "):
                    await lel.edit(
                        "<b>Remember to add Assistant bot to your Channel</b>",
                    )
                try:
    ...
    try:
        ...
        await lel.edit(
            f"<i> {user.first_name} got banned from this group, ask the admin to unban the assistant bot then add the Assistant Bot manually.</i>"
        )
        return
    ...
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"❌ **Videos with a duration of more than** `{DURATION_LIMIT}` **menit tidak boleh diputar!**"
            )
        ...
        try:
            ...
            await lel.edit(
                "**Admin Song not found.** Try searching with a clearer song title, Type `/help` if you need helphas been **updated"
            )
            print(str(e))
            return
        dlurl=url
...
#                    InlineKeyboardButton("⛑ Channel", url="https://t.me/TheKonoha11"),
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            photo="final.png",
            caption = f"🏷 **Title:** [{title[:60]}]({url})\n⏱ **Duration:** {duration}\n💡 **Status:** Q queue `{position}`\n" \
                    + f"🎼 **Request of:** {message.from_user.mention}",
...
async def deezer(client: Client, message_: Message):
    ...
    query = queryy
    res = lel
    await res.edit(f'**Searching for Songs** `{query}` **from deezer**')
    try:
...
async def jiosaavn(client: Client, message_: Message):
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    await USER.join_chat(invitelink)
                    await lel.edit(
                        f"<b>{user.first_name} successfully joined your group</b>",
                    )
    ...
    try:
        ...
        await lel.edit(
            "<i>Assistant Bot got banned from this group, ask the admin to unban the assistant bot then add the Assistant Bot manually.</i>"
        )
        return
    requested_by = message_.from_user.first_name
    chat_id = message_.chat.id
...
#                    InlineKeyboardButton("⛑ Channel", url="https://t.me/TheKonoha11"),
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"🎼 **The song you requested is Queuing in position** {position}",
        )

    else:
        await res.edit_text(f"🎼️ **Playing Song**")
        que[chat_id] = []
        qeue = que.get(chat_id)
        s_name = sname
        r_by = message_.from_user
    ...
    m = await client.send_photo(
        ...
        reply_markup=keyboard,
        photo="final.png",
        caption=f"🎼️ **Playing Song** {sname} **Via Jiosaavn**",
    )
    os.remove("final.png")

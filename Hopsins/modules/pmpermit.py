...
import asyncio
from Hopsins.config import SUDO_USERS, PMPERMIT, OWNER, PROJECT_NAME, UPDATES_CHANNEL, SUPPORT_GROUP
...
async def pmPermit(client: USER, message: Message):
    if PMPERMIT == "ENABLE" and PMSET:
        ...
        await USER.send_message(
            message.chat.id,
            f"Hello, I am **assistant service {PROJECT_NAME}.**\n\n ❗️ **Rules:**\n   - Don't spam messages here\n   - Don't spam songs so that it doesn't error\n   - Tutorial on how to use bot see on @{UPDATES_CHANNEL} \n\n 👉 **SEND AN INVITE LINK OR GROUP USERNAME, IF THE ASSISTANT CANNOT JOIN YOUR GROUP.**\n\n ⛑ **Group Support :** @{SUPPORT_GROUP} - **Owner** {OWNER}\n\n",
        )
        return
